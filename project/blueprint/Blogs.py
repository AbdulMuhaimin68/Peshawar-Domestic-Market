...
def get_all_blogs(args):
    id = args.get("blog_id")  # ✅ Correct key (lowercase "b")

    if id:
        try:
            res = BlogBLC.get_blogs_with_id(args)  # ✅ Expect a dictionary, not jsonify()
            
            if "error" in res:
                return jsonify(res), 404  # ✅ Return 404 if error

            return jsonify({"result": res}), 200  # ✅ Return 200 status
        except Exception as e:
            return jsonify({"error": str(e)}), 500  # ✅ Fix error format
    else:
        ...
        if res:
            ...
            return jsonify({"error": "No blogs found!"}), 404  # ✅ Return 404 for empty results
